...
### fix: 播放失败/加载卡顿/歌曲匹配/播放模式 — 4项核心修复

**commit: 2bd9345 + 3cc9ec7 + 07ccf04**

#### 问题1：播放失败（CDN URL过期403）
- **根因**：CDN签名URL约20分钟过期，但URL缓存TTL是1小时，缓存返回过期URL → CDN返回403
- **修复**：URL缓存TTL从1小时降至10分钟
- **修复**：audio-proxy上游403时返回410状态码，提示客户端URL已过期

#### 问题2：加载卡顿
- **根因**：togglePlay播放失败重试时未清空过期URL，反复用同一个过期URL
- **修复**：togglePlay失败时清空`cur.url = null`再重试，迫使play()重新获取

#### 问题3：歌曲与导入歌手不匹配
- **根因**：搜索"沧海一声笑 张晓红"返回罗大佑版本（热度更高），搜索按热度排序未验证歌手
- **修复**：song-url和resolveSongId搜索5首后优先精确匹配歌手名

#### 问题4：播放模式不生效
- **根因**：onended处理器忽略playMode，总是顺序切歌
- **修复**：添加playModeRef，onended根据playMode执行单曲循环/随机/顺序

#### 其他改进
- code=4重试改用name+artist参数（之前用合成ID `pl_0_xxx`，服务端无法识别）
- isTrial试听提示：非VIP歌曲显示"⏱ 试听30秒 · VIP可听完整版"
- isTrial标志同步到current状态
